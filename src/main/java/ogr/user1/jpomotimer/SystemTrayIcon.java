...
public class SystemTrayIcon {
    ...
    public static void reward() {
        SwingUtilities.invokeLater(() -> {
            JDialog rewardDialog = new JDialog();
            rewardDialog.setAlwaysOnTop(true);
            rewardDialog.setTitle("Reward cutie");
            rewardDialog.setSize(new Dimension(300, 300));
            rewardDialog.setLocationRelativeTo(null);
            rewardDialog.setDefaultCloseOperation(WindowConstants.DISPOSE_ON_CLOSE);

            JLabel rewardImageLabel = new JLabel();
            try {
                URL url = new URL("https://placekitten.com/300/300");
                ImageIcon rewardImageIcon = new ImageIcon(ImageIO.read(url));
                rewardImageLabel.setIcon(rewardImageIcon);
            } catch (IOException e) {
                e.printStackTrace();
            }
            rewardDialog.add(rewardImageLabel);
